...
total_sleep_week1 <- print(sum(week1_sleep))
...
total_sleep_week2 <- print(sum(week2_sleep))

## Determine if the total sleep in week 1 is less than week 2 by using the < operator
print(sum(week1_sleep)) < print(sum(week2_sleep))
...
c(weekdays1_mean) > c(weekdays2_mean)
...
rownames(grades, do.NULL = TRUE, prefix = "row")
rownames(grades) <- c(students)

colnames(grades, do.NULL = TRUE, prefix = "col")
